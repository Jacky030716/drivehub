...
<template>
  ...
  <section v-else class="sec-container">
    <div class="w-full h-full grid lg:grid-cols-5 grid-cols-3 gap-4 auto-rows-auto overflow-y-auto">
      <div class="relative md:row-span-3 md:col-span-2 col-span-full row-span-1 w-full h-full flex flex-col justify-center items-center bg-white gap-8 shadow-sm rounded-xl p-6">
      ...
      <div class="w-full h-full flex flex-col justify-center items-center gap-3.5 bg-white shadow-sm rounded-xl px-2 py-4 md:row-span-2">
        ...
        <p class="lg:w-[200px] text-center lg:text-base text-sm">You had shared <span class="font-bold text-primary">{{ linksSharedByMe }}</span> link(s) using DriveHub</p>
      </div>

      <div class="w-full h-full flex flex-col justify-center items-center gap-3.5 bg-white shadow-sm rounded-xl px-2 py-4 md:row-span-2">
        <img src="../assets/google-drive.png" alt="Share Icon" class="w-16 h-auto">
        <p class="lg:w-[200px] text-center lg:text-base text-sm">You had received <span class="font-bold text-primary">{{ linksSharedWithMe }}</span> link(s) from DriveHub</p>
      </div>

      <!-- Hubs details -->
      <div class="w-full h-full flex flex-col justify-center items-center gap-3.5 bg-white shadow-sm rounded-xl px-2 py-4 md:row-span-2">
        <img src="../assets/folder.png" alt="Share Icon" class="w-16 h-auto">
        <p class="lg:w-[200px] text-center lg:text-base text-sm">You had <span class="font-bold text-primary">{{ hubs.data.length }}</span> group(s) in DriveHub</p>
      </div>
      ...
      <div class="w-full h-full col-span-3 flex flex-col  gap-3.5 bg-white shadow-sm rounded-xl py-4 px-6">
        ...
        <div class="flex gap-3 flex-1 w-full justify-between">
          <RouterLink to="/share" class="flex flex-col items-center justify-center text-center h-auto border rounded-xl shadow-md p-4 flex-1 gap-2.5 hover:bg-slate-100 ease-in-out">
            ...
            <h3 class="font-semibold lg:text-base text-sm">Share a link</h3>
          </RouterLink>
          <RouterLink to="/shared" class="flex flex-col items-center justify-center text-center h-auto border rounded-xl shadow-md p-4 flex-1 gap-2.5 hover:bg-slate-100 ease-in-out">
            ...
            <h3 class="font-semibold lg:text-base text-sm">Views all links</h3>
          </RouterLink>
          <RouterLink to="/group" class="flex flex-col items-center justify-center text-center h-auto border rounded-xl shadow-md p-4 flex-1 gap-2.5 hover:bg-slate-100 ease-in-out">
            ...
            <h3 class="font-semibold lg:text-base text-sm">View all groups</h3>
          </RouterLink>
        </div>
      </div>
    </div>
